...
approves it with **Touch ID** and signs it in their own **Freighter** wallet. It also
...
- **Signing:** Rust Touch ID gate → **Freighter** bridge on a localhost page (`docs/freighter-bridge.md`); the app holds **no secret key**.
...
- **Touch ID + Freighter:** the user's biometric authorizes, their wallet signs; Polaris never custodies a key.
...
- **Card from the XDR:** the card renders the decoded transaction, not the model's text; the bridge re-verifies the signed envelope.
...
- **Wired in `integration/wallet`:** voice pipeline, panels/tray, approval card, Touch ID gate, Freighter bridge, guard limits, schedules, P2P client, SPP read-only.
- **Needs a human/deployment:** live mic + Touch ID + Freighter round trip, the P2P contract, the anchor payout (anchor-side), value-moving SPP.
- **Designed, not shipped:** Soroswap/DeFindex, MPP, passkey wallet, developer mode.

## Roadmap
Protocol integration and MPP; value-moving SPP once the bridge can sign auth entries;
