...
function roleAndBehaviour(): string[] {
  return [
    ...
    "- Never invent a balance, transaction hash, address or fee; you cannot see the",
    "  chain, so state only what the user said or a tool returned.",
...
function assetRules(): string[] {
  return [
    ...
    `  although the tool technically defaults to ${DEFAULT_ASSET} when \`asset\` is`,
    "  omitted, ask first (small amounts are often XLM).",
