...
## Round 3 — MERGE-FIX3: merge the two reviewed W5a/W5b fix branches (2026-09-20)
- `feat/w5a-anchor-signing`: resolved `bridge/verify.rs` by keeping the review fix's panic-free parser (fallible slicing, `find_signature_list` requires `list_start` past the header, crafted-XDR + mutation/fuzz tests) **and** integration's later `SourceMismatch` check, detail and test; docs kept both sides' rows.
- `feat/w5b-anchor-flows`: resolved `chain.ts` by keeping W6b's `schedulesLive` tools and adding the W5b early route (`deposit`/`withdraw` → `runAnchorIntent`, i.e. the panel's signer routing); F1 `onStage`, web logging and the Touch ID approver are intact.
- One integration seam the review test exposed: `defaultSignViaPipeline` now passes a `resequence` passthrough to `runTx`, so partial test seams no longer load the heavy default approver/signer (single-tx `runTx` never resequences).
- Files: `app/src-tauri/src/bridge/verify.rs`, `app/src/lib/anchor.ts`, `app/src/lib/chain.ts`, `backlog.md`, `sprints.md` (plus branch-added TS tests/panels auto-merged).
- Verification: `npm run check` clean; app **301/0**, agent **179/0**, stellar **all suites pass (67 node + 963 vitest) / 0 fail**; `npm run build -w @polaris/app` OK; `cargo test` **310 pass / 0 fail / 5 ignored**; `cargo clippy --all-targets -- -D warnings` clean.
- Security untouched: fail-closed approver, `WalletOnly` reachable only in-process, `POLARIS_ALLOW_AUTO_APPROVE` never enabled by this code. Human-verify: live Freighter/Touch ID anchor round trip (mic/real windows not exercised).
