...
| 2026-09-20 | **W5a — anchor SEP-10 wallet-only signing (Rust)**: `bridge_sign_challenge` refuses any non-seq-0/owner-sourced/unsigned XDR, authorizes via in-process `begin_wallet_only`/`authorize_wallet_only`, verifies the challenge (same body, one added owner signature, `manage_data`-only); generalised signature-list parsing (0–4 sigs); `anchor_signing_health`; real Freighter round trip still needs a human | opencode worker (deepseek-v4.1-flash) | open (review) | `backlog/w5a-anchor-signing.md` | P0 |
